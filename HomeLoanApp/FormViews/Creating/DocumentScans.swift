...
struct DocumentScans: View {
    ...
    var body: some View {
        Form() {
            ...
            Section() {
                Text("\(applicationCreation.application.loanID?.uuidString ?? "nil")")
    ...
    private func hasScanned() -> Bool {
        ...
        if identityType == "Passport" || identityType == "ID Book" { // When the clients ID type is a Passport (or ID Book) they must scan just one side
            if let documentsDirectory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first {
                let fileName = "passport_id_image_\(applicationCreation.application.loanID?.uuidString ?? "nil")_0.png"
            ...
            if let documentsDirectory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first {
                ...
                for scanNumber in 0..<2 {
                    let fileName = "passport_id_image_\(applicationCreation.application.loanID?.uuidString ?? "nil")_\(scanNumber).png"
                    let fileURL = documentsDirectory.appendingPathComponent(fileName)
                    if scanNumber == 0 {
    ...
    private func scanString() -> String {
        ...
        if let idType = applicationCreation.application.iDType {
            if !idType.isEmpty {
